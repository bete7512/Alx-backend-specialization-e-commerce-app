<template>
  ...
  <div v-else class="flex justify-center items-center">
    <div
      class="container h-full bg-zinc-100 w-full px-4 shadow-slate-800 sm:w-3/4 md:1/2 lg:1/2 space-y-3"
    >
      ...
      <div class="space-y-8">
        <div
          class="font-bold flex-wrap"
          v-for="(option, index) in result.product_by_pk.p_options"
          :key="option.id"
        >
          ...
          <div
            class="flex flex-wrap space-x-2 justify-center items-center py-3"
          >
            <button
              :class="['bg-blue-500']"
              @click="add_cart(option.id)"
            ...
            >
              ...
              <div v-if="!option.is_user_added_to_order">Order</div>
              <div v-else="option.is_user_added_to_order">Ordered</div>
            </button>
          </div>
        </div>
      </div>
...
const { error, loading, result } = useQuery(GET_PRODUCT_DETAIL, {
  id: route.params.id,
});
</script>
<style></style>
