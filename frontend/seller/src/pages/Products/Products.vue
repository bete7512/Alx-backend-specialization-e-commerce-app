<template>
  <div class="overflow-x-scroll">
    <div class="flex justify-between">
      ...
      <button
        @click="modal = true"
        class="text-green-900 p-4 mb-2 capitalize rounded-lg bg-orange-400"
      >
        add new product
      </button>
    </div>
    ...
    >
      ...
      <tbody class="px-2">
        ...
        >
          ...
          <td class="py-3 px-2">
            {{ pro.product_reviews_aggregate.aggregate.count }}
          </td>
            <td class="py-3 px-2">{{ pro.quantity }}</td>           
  ...
  <ProductDetail
    :id="id_param"
    v-if="detail_product"
    v-on:close="detail_product = false"
  ></ProductDetail>
  <Delete
    v-if="isDelete"
    v-on:canceldelete="isDelete = false"
    :id="id_param"
    v-on:deletenotify="
      () => {
        isDelete = false;
...
const { error, result, loading, refetch } = useQuery(PRODUCT_QUERY);
